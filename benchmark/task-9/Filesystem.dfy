  ...
  class Files {
    ...
    */
    ...
    method OpenWithAccessMode(file: string, accessMode: Access) returns (res: Result<object, string>)
      ...
      requires |file| > 0
      ensures res.Success? ==> is_open
      ensures res.Success? ==> access == if (file == "~/protected-file.txt") then Access.Read else accessMode
      ensures res.Success? ==> appendMode == is_open
    {
      var isError, fileStream, errorMsg := INTERNAL_Open(file);
      is_open := !isError;
      if file == "~/protected-file.txt" {
        this.access := Access.Read;
      } else {
        this.access := accessMode;
        assert this.access == accessMode;
      }
      this.appendMode := is_open;
      return if (isError) then Failure(errorMsg) else Success(fileStream);
    }
    ...
    method AppendBytesToFileWithAccessMode(file: string, bytes: seq<bv8>, accessMode: Access) returns (res: Result<(), string>)
    requires this.is_open && this.access == Access.Write && accessMode == this.access && this.appendMode
    {
      var isError, errorMsg := INTERNAL_AppendBytesToFile(file, bytes);
      return if isError then Failure(errorMsg) else Success(());
    }
    ...
    method
    {:extern "Filesystem.Files", "INTERNAL_AppendBytesToFile"}
    INTERNAL_AppendBytesToFile(file: string, bytes: seq<bv8>)
    returns (isError: bool, errorMsg: string)
