...
class default__:
    ...
    def Constant(cs, expected):
        d_749_valueOrError0_ = (cs).AssertBytes(expected, 0)
        if (d_749_valueOrError0_).IsFailure():
            return (d_749_valueOrError0_).PropagateFailure()
        elif True:
            d_750_cs_ = (d_749_valueOrError0_).Extract()
            return Std_Wrappers.Result_Success((d_750_cs_).Split())
