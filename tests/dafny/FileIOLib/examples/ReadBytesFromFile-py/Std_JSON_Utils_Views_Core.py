...
class default__:
    ...
    @staticmethod
    def Merge(lv, rv):
        d_364_dt__update__tmp_h0_ = lv
        d_365_dt__update_hend_h0_ = (rv).end
        return View___View((d_364_dt__update__tmp_h0_).s, (d_364_dt__update__tmp_h0_).beg, d_365_dt__update_hend_h0_)
...
class View__:
    ...
    @staticmethod
    def OfString(s):
        return _dafny.SeqWithoutIsStrInference([ord((s)[d_366_i_]) for d_366_i_ in range(len(s))])
    ...
    def CopyTo(self, dest, start):
        hi0_ = (self).Length()
        for d_367_idx_ in range(0, hi0_):
            index6_ = (start) + (d_367_idx_)
            (dest)[index6_] = ((self).s)[((self).beg) + (d_367_idx_)]
