...
class default__:
    ...
    def String(st, byte):
        ...
        if source13_.is_Start:
            ...
            d_373___mcc_h0_ = source13_.escaped
            d_374_escaped_ = d_373___mcc_h0_
            if (byte) == (ord(_dafny.CodePoint('\\'))):
                return Std_JSON_Utils_Lexers_Core.LexerResult_Partial(StringLexerState_Body(not(d_374_escaped_)))
            elif ((byte) == (ord(_dafny.CodePoint('\"')))) and (not(d_374_escaped_)):
