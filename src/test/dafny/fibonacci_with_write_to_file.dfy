...
method FibonacciToByteSequence(n: nat) returns (byteSeq: seq<byte>)
  ensures byteSeq == StringToBytes(NumberToString(Fib(11)))
{
  var fibo := ComputeFibonacci(11);
  assert Fib(11) == fibo;

  var fibString := ComputeNumberToString(fibo);
  assert fibString == NumberToString(Fib(11));
...
method IsFileInSet(filename: string, files: set<string>) returns (found: bool)
