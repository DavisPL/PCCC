...
class Chain:
    ...
    def Length(self):
        d_368___accumulator_ = 0
        _this = self
        while True:
            with _dafny.label():
                if (_this).is_Empty:
                    return (0) + (d_368___accumulator_)
                elif True:
                    d_368___accumulator_ = (((_this).v).Length()) + (d_368___accumulator_)
    ...
    def Count(self):
        d_369___accumulator_ = 0
        _this = self
        while True:
            with _dafny.label():
                if (_this).is_Empty:
                    return (0) + (d_369___accumulator_)
                elif True:
                    d_369___accumulator_ = (1) + (d_369___accumulator_)
    ...
    def Bytes(self):
        d_370___accumulator_ = _dafny.SeqWithoutIsStrInference([])
        _this = self
        while True:
            with _dafny.label():
                if (_this).is_Empty:
                    return (_dafny.SeqWithoutIsStrInference([])) + (d_370___accumulator_)
                elif True:
                    d_370___accumulator_ = (((_this).v).Bytes()) + (d_370___accumulator_)
    ...
    def CopyTo(self, dest, end):
        ...
        while True:
            with _dafny.label():
                if (_this).is_Chain:
                    d_371_end_: int
                    d_371_end_ = (end) - (((_this).v).Length())
                    ((_this).v).CopyTo(dest, d_371_end_)
                    in67_ = (_this).previous
                    in68_ = dest
                    in69_ = d_371_end_
                    _this = in67_
