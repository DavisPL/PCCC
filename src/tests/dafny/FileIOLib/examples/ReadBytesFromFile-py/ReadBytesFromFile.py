...
class default__:
    ...
    @staticmethod
    def Test():
        default__.theMain(_dafny.SeqWithoutIsStrInference(map(_dafny.CodePoint, "/Users/pari/pcc-llms/src/tests/dafny/FileIOLib/examples/../data.txt")), _dafny.SeqWithoutIsStrInference(map(_dafny.CodePoint, "")))
    ...
    def theMain(dataPath, expectedErrorPrefix):
        if True:
            ...
            if not((d_811_res_).is_Success):
                raise _dafny.HaltException("tests/dafny/FileIOLib/examples/ReadBytesFromFile.dfy(60,6): " + ((_dafny.SeqWithoutIsStrInference(map(_dafny.CodePoint, "unexpected failure: "))) + ((d_811_res_).error)).VerbatimString(False))
            ...
            if not((d_812_readBytes_) == (d_809_expectedBytes_)):
                raise _dafny.HaltException("tests/dafny/FileIOLib/examples/ReadBytesFromFile.dfy(63,6): " + (_dafny.SeqWithoutIsStrInference(map(_dafny.CodePoint, "read unexpected byte sequence"))).VerbatimString(False))
        if True:
            ...
            if not((d_814_res_).is_Failure):
                raise _dafny.HaltException("tests/dafny/FileIOLib/examples/ReadBytesFromFile.dfy(69,6): " + (_dafny.SeqWithoutIsStrInference(map(_dafny.CodePoint, "unexpected success"))).VerbatimString(False))
            if not((expectedErrorPrefix) <= ((d_814_res_).error)):
                raise _dafny.HaltException("tests/dafny/FileIOLib/examples/ReadBytesFromFile.dfy(70,6): " + ((_dafny.SeqWithoutIsStrInference(map(_dafny.CodePoint, "unexpected error message: "))) + ((d_814_res_).error)).VerbatimString(False))
